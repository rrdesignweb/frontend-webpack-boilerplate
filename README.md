...
* Edit the [`configuration/environment.js`](configuration/environment.js) if you want to specify:
  ...
  * **`paths`**: `src` or `build` directories names and file system location.
...
* _JavaScript_ files with support of _ES6 / ECMAScript 2016(ES7)_ files are located under `/build/js/`
* _Images_ are located under `/build/images/`
  * Images part of the _design_ (_usually referenced in the CSS_) are located under `/build/images/design/`
  * Images part of the _content_ (_usually referenced via `<img>` tags_) are located under `/build/images/content/`
